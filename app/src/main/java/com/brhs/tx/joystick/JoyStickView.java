...
public class JoyStickView extends View {
    ...
    private void initJoystickView() {
        ...
        bgPaint.setColor(getContext().getColor(R.color.appJoyStickOuterColor));
        bgPaint.setStrokeWidth(4);
        bgPaint.setStyle(Paint.Style.STROKE);

        handlePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        handlePaint.setColor(getContext().getColor(R.color.appJoyStickInnerColor));
        handlePaint.setStrokeWidth(4);
        handlePaint.setStyle(Paint.Style.FILL_AND_STROKE);

        innerPadding = 10;

        setMovementRange(JoyStickConstants.JOYSTICK_MOVEMENT_RANGE);
        setMoveResolution(JoyStickConstants.JOYSTICK_MOVEMENT_RESOLUTION);
    ...
    public void setPreferences(SharedPreferences prefs) {
        prefRatio = Float.parseFloat(prefs.getString("50", "50"));
        prefRatio /= 100.0;
        requestLayout();
    }
...
//        canvas.save();
    ...
    public void autoReturn(boolean immediate) {
        if (autoReturnMode != AUTO_RETURN_NONE) {
            final int numberOfFrames = immediate ? 1 : 1;
